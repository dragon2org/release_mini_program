...
class ReleaseDetailItemsTransformer extends TransformerAbstract
{
    public function transform(ReleaseItem $item)
    {
        return [
            ...
            'name' => $item->name,
            "original_config" => $item->original_config ? json_decode($item->original_config, true) : [],
            "online_config" => $item->online_config? json_decode($item->online_config, true) : [],
            "push_config" => $item->push_config ? json_decode($item->push_config, true) : [],
